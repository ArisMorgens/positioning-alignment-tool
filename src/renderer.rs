//! Minimal 3D scene renderer for the live position view.
//!
//! Trimmed down from swarmkeeper's `renderer.rs`: draws a ground grid, XYZ axes,
//! the current Crazyflie position (with a drop-line to the floor), and a trail
//! of recent positions as a line strip.
...
/// A point to draw in the scene (current Crazyflie position).
#[derive(Clone, Copy)]
pub struct UnitPos {
    pub x: f32,
    pub y: f32,
...
impl Scene3DRenderer {
    ...
    /// Render the live position scene: ground grid, XYZ axes, the current
    /// position (if any) with a drop-line to the floor, and a trail of
    /// recent positions as a line strip.
    pub fn render(
        &mut self,
        width: u32,
        height: u32,
        ...
        unsafe {
            ...
            for i in -grid_range..=grid_range {
                let v = i as f32;
                let r = grid_range as f32;
                grid_verts.extend_from_slice(&[v, -r, 0.0, v, r, 0.0]);
                grid_verts.extend_from_slice(&[-r, v, 0.0, r, v, 0.0]);
            }
            ...
            if trail_lines.len() >= 6 {
                ...
                gl.uniform_1_f32(Some(&self.u_point_size), 1.0);
                upload_and_draw(gl, self.vbo, trail_lines);
                gl.draw_arrays(glow::LINE_STRIP, 0, (trail_lines.len() / 3) as i32);
            }

            // Draw current position as a point with a drop-line to the floor
            if let Some(pos) = current {
                gl.uniform_1_f32(Some(&self.u_point_size), 12.0);
                gl.uniform_3_f32(Some(&self.u_color), pos.color[0], pos.color[1], pos.color[2]);
                ...
                gl.uniform_1_f32(Some(&self.u_point_size), 1.0);
                let drop_line = [pos.x, pos.y, pos.z, pos.x, pos.y, 0.0];
                upload_and_draw(gl, self.vbo, &drop_line);
                gl.draw_arrays(glow::LINES, 0, 2);
            }
